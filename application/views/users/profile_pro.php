...
?>        
        <div id="inner_container">
            ...
            <div id="large_profile">
                              ...
                              <?php if(count($image_slide)>0){?>
                                      <div id="gallery_container">
                                                ...
                                                <div style="margin-left:13px;">
                                                  <?php
                                                  $image_slide = $udetail[0]['image_data']; 
                                                  for($j=0;$j<count($image_slide);$j++) {
                                                  ?>
                                                    <div id="gallery_image_container">
                                                      <img src="<?php echo $image_slide[$j]?>" />
                                                    </div>
                                                  <?php } ?>                                                
                                                </div>
                                                <div class="clearfloat"></div>
                ...
                for($k=0;$k<count($db_other_service);$k++) {?>                      
                <div id="listing_card_small_profile">
              		<div id="listing_card_small_details_container">
                    		<div id="listing_card_small_name"><a href="user/prfile_pro/<?echo $db_other_service[$k]['iCompanyServiceId']?>"><?echo $db_other_service[$k]['vCompanyName']?></a></div>
                            <div class="clearfloat"></div>
                            <div id="listing_card_small_location" ><?echo $db_other_service[$k]['vCity']?></div>
                            <div id="listing_card_small_location3" >,</div>
                            <div id="listing_card_small_location2" ><?php echo (isset($db_other_service[$k]['vState']))? $db_other_service[$k]['vState'].',':'' ?><?php echo $db_other_service[$k]['vCountry']?></div>
                            <div class="clearfloat"></div>
                            <div id="listing_card_small_profession"><?echo $db_other_service[$k]['vServiceName']?></div>
                    </div>
                    <div class="clearfloat"></div>
                    <?php
                        if(file_exists($this->config->config['upload_path'].$db_other_service[$k]['vImage'])) {
                            $db_other_service[$k]['vImage'] = $this->config->config['upload_url'].$db_other_service[$k]['vImage'];
                        } else {
